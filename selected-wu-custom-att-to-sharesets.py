...
def get_selected_object_ids(client):
    """Return IDs for objects currently selected in the Wwise UI."""
    selected = client.call(
        "ak.wwise.ui.getSelectedObjects",
        {},
        options={"return": ["id"]},
    )
    return [obj["id"] for obj in selected.get("objects", []) if "id" in obj]
...
def main():
    ...
    try:
        with WaapiClient() as client:
            selected_ids = get_selected_object_ids(client)
            if not selected_ids:
                logging.warning("No objects selected in Wwise UI.")
                return

            selected_ids_waql = ", ".join(f'"{selected_id}"' for selected_id in selected_ids)
            custom_attenuation_waql = (
                f"$ {selected_ids_waql} select descendants select attenuation where owner != null"
            )
